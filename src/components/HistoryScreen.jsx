            ...
            history.map((workout, i) => {
              ...
              const totalVolume = workout.exercises.reduce((acc, ex) =>
                acc + ex.sets.filter(s => s.completed).reduce((sAcc, s) =>
                  sAcc + (s.weight || 0) * (s.reps || 0), 0), 0);

              return (
                <div key={workout.id || i} className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 mb-4 border border-white/20 hover:bg-white/15 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-white">{workout.name}</h3>
                      <div className="text-sm text-white/60">{new Date(workout.date || workout.startTime).toLocaleDateString()} • {Math.round((workout.duration || 0) / 60000)} min</div>
                    </div>
                    <div className="text-right">
                      <div className="text-cyan-400 font-medium">{totalSets} sets</div>
                      <div className="text-sm text-white/60">{totalVolume.toLocaleString()} lbs</div>
                    </div>
                  </div>
